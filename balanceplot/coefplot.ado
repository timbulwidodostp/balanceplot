*! version 1.7.5  29jan2015  Ben Jann
...
program _coefplot, rclass
    ...
    forv j = 1/`n_subgr' {
        ...
        if ("`recycle'"=="" & `n_subgr'>1) {
            combine_plotopts, `opts' _opts0(`plotopts1prev')
            local plotopts1prev `plotopts'
            local opts `plotopts' `options'
        }
        ...
        forv k = 1/`n_plots_`j'' {
            ...
            if ("`recycle'"=="" & `n_subgr'>1) { // 
                combine_plotopts, `opts' _opts0(`plotopts2_`i'')
                local plotopts2_`i' `plotopts'
                local opts `plotopts' `options'
            }
    ...
    tempname by plot at mlbl mlpos b V se df eq grp
    qui gen `by' = .
    qui gen `plot' = .
    qui gen `at' = .
    ...
    if `"`generate'"'!="" {
        ...
        local returnvars
        local i 0
        foreach v in by plot at mlbl mlpos b V se df {
            local ++i
        ...
        forv i = 1/`N_ci' {
            foreach v in ll ul {
                if "`v'"=="ll" local varl "CI`i': lower limits"
                else           local varl "CI`i': upper limits"
                if "`replace'"!="" {
        ...
        if (`n_ci')>0 {
            ...
            forv k = 1/`n_ci' {
                ...
                local ciplots `ciplots' ///
                    (rspike `ll`k'' `ul`k'' `at' if `plot'==`i'`ifopt_`i'', ///
...
program parse_globalopts
    ...
    if `"`coeflabels'"'!="" {
        parse_coeflabels `coeflabels'
            // returns coeflabels, cltrunc, clwrap, clbreak, clinteract, clopts
    }
    if `"`clinteract'"'=="" {
        local clinteract `"" # ""'
    }
    ...
    foreach opt in                                              ///
        ...
        format                                                  ///
        labels                                                  ///
        coeflabels cltrunc clwrap clbreak clinteract clopts     ///
        noeqlabels                                              ///
...
program merge_plotopts
    merge_modelopts `0' // returns modelopts, mlabels2, options, _opts0
    _merge_plotopts, `options'
    _merge_plotopts _opt0_, `_opts0'
    if `"`mlabels'"'!="" local _opt0_mlabel
    if `"`mlabel'"'!=""  local _opt0_mlabels
    ...
    c_local modelopts2 `modelopts' `mlabels2'
    c_local plotopts `0'
    c_local options `options'
...
program collect_coefs
    ...
    // get options
    merge_modelopts `0' // returns modelopts, mlabels2, options, _opts0
    local 0 , `modelopts' `mlabels2' `options' `_opts0'
    syntax [,                       ///
        OMITted                     ///
        BASElevels                  ///
        Bname(str)                  ///
...
struct coefplot_struct
{
    ...
    real colvector   b, V, se, df, at, plot, by, eq, grp, mlpos
    real matrix      ci, aux
...
void coefplot_keepdrop(struct coefplot_struct scalar C)
{
    real scalar         i, j, level, r, row, col, emode, firsteqonly, meqs, 
                        cilogit
    real colvector      b, p, at, V, se, df, mlpos
    real matrix         ci, aux, tmp
    string scalar       model, cname, rename, attrans, attmp
    ...
    if (cname=="") {
        if (st_global("e(mi)")=="mi") cname = "b_mi"
        else                          cname = "b"
    }
    ...
    for (j=1; j<=cols(cnames); j++) {
        cname = cnames[j]
        coefplot_parse_input(model, "aux", cname, row, col)
        if (emode) cname = "e(" + cname + ")"
        tmp = st_matrix(cname)
        if (coefplot_notfound(model, cname, tmp)==0) {
            coefplot_invalid_subscript(model, cname, tmp, row, col)
    ...
    real scalar naux)
{
    ...
    nspace = nspace, ("b", "V", "se", "df", "at", "mlbl", "mlpos", "plot", "by")
